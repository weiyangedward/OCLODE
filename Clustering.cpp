...
double Clustering::get_coexpress_cost(SpeciesNetwork **species_network)
{
    ...
    for (int spe = 0; spe < num_species; spe++)
    {
        ...
        for (map<int, int>::iterator x = total_cluster_nodes.begin(); x != total_cluster_nodes.end(); x++)
        {
...
//    fprintf(stderr, "===== get_orth_delta_cost spe %d, node %s ======\n", spe, (species_network[spe]->uniqId_to_geneName[perturb_node]).c_str());
    ...
    for (unordered_set<string>::iterator i = orthology->spe_orth_node_adjList[spe][perturb_node].begin(); i!=orthology->spe_orth_node_adjList[spe][perturb_node].end();i++)
    {
        string x = *i;
